...
#define FOREACH(ss, vv) \
  for (std::vector<std::string>::iterator ss = vv.begin(); ss != vv.end(); ++ss)
...
struct TimerType : PVR_TIMER_TYPE
{
            ...
            int dupEpisodesDefault)
  {
    ...
    for (auto it = maxRecordingsValues.begin(); it != maxRecordingsValues.end(); ++it, ++i)
    {
      ...
      strncpy(maxRecordings[i].strDescription, it->second.c_str(),
              sizeof(maxRecordings[i].strDescription) - 1);
    }
...
PVR_ERROR Pvr2Wmc::GetTimerTypes(PVR_TIMER_TYPE types[], int* size)
{
  ...
  if (lifetimeValues.size() == 0)
  {
    ...
    lifetimeValues.push_back(
        std::make_pair(WMC_LIFETIME_ELIGIBLE, XBMC->GetLocalizedString(30163)));
  ...
  {
    recordingLimitValues.push_back(
        std::make_pair(WMC_LIMIT_ASMANY, XBMC->GetLocalizedString(30170)));
  ...
  {
    showTypeValues.push_back(
        std::make_pair(WMC_SHOWTYPE_FIRSTRUNONLY, XBMC->GetLocalizedString(30150)));
    showTypeValues.push_back(std::make_pair(WMC_SHOWTYPE_ANY, XBMC->GetLocalizedString(30151)));
    showTypeValues.push_back(
        std::make_pair(WMC_SHOWTYPE_LIVEONLY, XBMC->GetLocalizedString(30152)));
  }
...
PVR_ERROR Pvr2Wmc::GetChannelGroupMembers(ADDON_HANDLE handle, const PVR_CHANNEL_GROUP& group)
{
  ...
  std::string request;
  request = string_format("GetChannelGroupMembers|%s|%s", group.bIsRadio ? "True" : "False",
                          group.strGroupName);
  vector<std::string> results = _socketClient.GetVector(request, true);
...
PVR_ERROR Pvr2Wmc::GetEPGForChannel(ADDON_HANDLE handle,
                                    int iChannelUid,
                                    time_t iStart,
                                    time_t iEnd)
{
  ...
  std::string request;
  request = string_format("GetEntries|%u|%lld|%lld", iChannelUid, (long long)iStart,
                          (long long)iEnd); // build the request string
  ...
  FOREACH(response, results)
  {
    ...
    xEpg.strPlotOutline =
        v[5].c_str(); // short plot description (currently using episode name, if there is one)
...
PVR_ERROR Pvr2Wmc::AddTimer(const PVR_TIMER& xTmr)
{
  ...
  {
    XBMC->Log(LOG_DEBUG, "recording added for timer '%s', with rec state %s", xTmr.strTitle,
              results[0].c_str());
    ...
    if (results.size() > 1) // if there is extra results sent from server...
    {
      ...
      {
        vector<std::string> splitResult =
            split(*result, "|"); // split to unpack extra info on each result
        std::string infoStr;
        ...
        {
          XBMC->Log(LOG_DEBUG, "instant record end time increased by '%s' minutes",
                    splitResult[1].c_str());
...
std::string Pvr2Wmc::Timer2String(const PVR_TIMER& xTmr)
{
  ...
  bool bRepeating =
      xTmr.iTimerType >= TIMER_REPEATING_MIN && xTmr.iTimerType <= TIMER_REPEATING_MAX;
  bool bKeyword = xTmr.iTimerType == TIMER_REPEATING_KEYWORD ||
                  xTmr.iTimerType == TIMER_ONCE_KEYWORD ||
                  xTmr.iTimerType == TIMER_ONCE_KEYWORD_CHILD;
  bool bManual = xTmr.iTimerType == TIMER_ONCE_MANUAL ||
                 xTmr.iTimerType == TIMER_ONCE_MANUAL_CHILD ||
                 xTmr.iTimerType == TIMER_REPEATING_MANUAL;

  //was ("|%d|%d|%d|%d|%d|%s|%d|%d|%d|%d|%d",
  tStr = string_format(
      "|%u|%d|%lld|%lld|%d|%s|%d|%u|%u|%d|%u", xTmr.iClientIndex, xTmr.iClientChannelUid,
      (long long)xTmr.startTime, (long long)xTmr.endTime, PVR_TIMER_STATE_NEW, // 0-4
  ...
  extra = string_format("|%u|%d|%u|%d|%d|%d|%s|%d|%d", xTmr.iPreventDuplicateEpisodes,
                        xTmr.bStartAnyTime, xTmr.iWeekdays, // 11-13 param
                        xTmr.iLifetime, bKeyword, xTmr.bFullTextEpgSearch, xTmr.strEpgSearchString,
                        xTmr.iMaxRecordings, bManual); // 14-19
  tStr.append(extra);
...
PVR_ERROR Pvr2Wmc::DeleteTimer(const PVR_TIMER& xTmr, bool bForceDelete)
{
  ...
  {
    XBMC->Log(LOG_DEBUG, "deleted timer '%s', with rec state %s", xTmr.strTitle,
              results[0].c_str());
    return PVR_ERROR_NO_ERROR;
  }
}
...
PVR_ERROR Pvr2Wmc::GetTimers(ADDON_HANDLE handle)
{
  ...
  FOREACH(response, responsesSeries)
  {
    ...
    xTmr.iEpgUid =
        atoi(v[3].c_str()); // [3] epg ID (same as client ID, except for a 'manual' record)
    ...
    xTmr.iWeekdays =
        atoi(v[15].c_str()); // [15] DaysOfWeek (converted to Kodi values in the backend)
    ...
    xTmr.iTimerType = hasKeyword ? TIMER_REPEATING_KEYWORD
  ...
  FOREACH(response, responsesTimers)
  {
    ...
    xTmr.iWeekdays =
        atoi(v[22].c_str()); // [22] DaysOfWeek (converted to Kodi values in the backend)
    ...
    {
      xTmr.iTimerType =
          hasKeyword ? TIMER_ONCE_KEYWORD : hasEPG ? TIMER_ONCE_EPG : TIMER_ONCE_MANUAL;
    }
...
PVR_ERROR Pvr2Wmc::DeleteRecording(const PVR_RECORDING& recording)
{
  ...
  std::string command;
  command =
      string_format("DeleteRecording|%s|%s|%s", recording.strRecordingId, recording.strTitle, "");
...
PVR_ERROR Pvr2Wmc::SetRecordingLastPlayedPosition(const PVR_RECORDING& recording,
...
std::string Pvr2Wmc::Channel2String(const PVR_CHANNEL& xCh)
{
  // packing: id, bradio, c.OriginalNumber, c.CallSign, c.IsEncrypted, imageStr, c.IsBlocked
  std::string chStr;
  chStr = string_format("|%u|%d|%u|%s", xCh.iUniqueId, xCh.bIsRadio, xCh.iChannelNumber,
                        xCh.strChannelName);
  return chStr;
}
...
bool Pvr2Wmc::OpenLiveStream(const PVR_CHANNEL& channel)
{
  ...
  else
  {
    ...
    if (results.size() > 1)
      XBMC->Log(LOG_DEBUG, "OpenLiveStream> opening stream: %s",
    ...
    _streamFile = XBMC->OpenFile(_streamFileName.c_str(),
                                 0); // open the video file for streaming, same handle
...
int Pvr2Wmc::ReadLiveStream(unsigned char* pBuffer, unsigned int iBufferSize)
{
  ...
  if (!_streamWTV) // if NOT streaming wtv, make sure stream is big enough before it is read
  {
    ...
    if (_initialStreamPosition > 0 && PositionLiveStream() == 0)
    {
      ...
      {
        XBMC->Log(LOG_DEBUG, "ReadLiveStream> stream file seek to initial position %llu successful",
                  _initialStreamPosition);
      }
      else
      {
    ...
    while (_isStreamFileGrowing && currentPos + iBufferSize > fileSize)
    {
               ...
               -1) // if fileSize -1, server is reporting an 'unkown' error with the stream
      {
        XBMC->QueueNotification(
            QUEUE_ERROR, XBMC->GetLocalizedString(30003)); // display generic error with stream
      ...
      if (timeout > 50) // if after 30 sec the file has not grown big enough, timeout
      {
        ...
        {
          XBMC->QueueNotification(
              QUEUE_ERROR, XBMC->GetLocalizedString(30003)); // otherwise display generic error
...
long long Pvr2Wmc::ActualFileSize(int count)
{
  ...
  else
  {
    ...
    if (lFileSize <
        -1) // if server returns a negative file size, it means the stream file is no longer growing (-1 => error)
    {
...
bool Pvr2Wmc::OpenRecordedStream(const PVR_RECORDING& recording)
{
  ...
  vector<std::string> results = _socketClient.GetVector(
  ...
  else
  {
    ...
    if (results.size() > 1)
      XBMC->Log(LOG_DEBUG, "OpenRecordedStream> rec stream type: %s",
                results[1].c_str()); // either a 'passive' or 'active' WTV OR a TS file

    if (results.size() > 2)
      XBMC->Log(LOG_DEBUG, "OpenRecordedStream> opening stream: %s",
  ...
  if (_signalStatusCount-- <= 0)
  {
    ...
    else
    {
      if (results.size() >= 9)
      {
        ...
        snprintf(cachedSignalStatus.strAdapterName, sizeof(cachedSignalStatus.strAdapterName), "%s",
                 results[0].c_str());
        snprintf(cachedSignalStatus.strAdapterStatus, sizeof(cachedSignalStatus.strAdapterStatus),
                 "%s", results[1].c_str());
        snprintf(cachedSignalStatus.strProviderName, sizeof(cachedSignalStatus.strProviderName),
                 "%s", results[2].c_str());
        snprintf(cachedSignalStatus.strServiceName, sizeof(cachedSignalStatus.strServiceName), "%s",
                 results[3].c_str());
        snprintf(cachedSignalStatus.strMuxName, sizeof(cachedSignalStatus.strMuxName), "%s",
                 results[4].c_str());
...
PVR_ERROR Pvr2Wmc::GetRecordingEdl(const PVR_RECORDING& recording,
                                   PVR_EDL_ENTRY entries[],
                                   int* size)
{
