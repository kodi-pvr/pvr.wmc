...
bool Socket::create()
{
  ...
  if (_timeout != 0)
  {
    ...
    setsockopt(_sd, SOL_SOCKET, SO_RCVTIMEO, (char*)&tv,
               sizeof(struct timeval)); // set a receive timeout for this new socket
...
std::vector<std::string> Socket::GetVector(const std::string& request,
                                           bool allowRetry,
                                           bool allowWOL /* = true*/)
{
  int maxAttempts = 3;
