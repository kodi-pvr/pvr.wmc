...
extern "C"
{
  ...
  void ADDON_ReadSettings(void)
  {
    ...
    {
      XBMC->Log(LOG_ERROR, "Couldn't get 'wake_on_lan' setting, using '%s'",
    ...
    if (ReadFileContents(g_AddonDataCustom, fileContent))
    {
      ...
      XBMC->Log(LOG_ERROR, "Using ServerWMC MAC address from custom addondata '%s'",
                g_strServerMAC.c_str());
    }
    else
    {
      XBMC->Log(LOG_ERROR,
    ...
    {
      XBMC->Log(LOG_ERROR, "Couldn't get 'signal_throttle' setting, using '%s'",
  ...
  ADDON_STATUS ADDON_SetSetting(const char* settingName, const void* settingValue)
  {
    ...
    if (sName == "host")
    {
      ...
      XBMC->Log(LOG_INFO, "Setting 'host' changed from %s to %s", g_strServerName.c_str(),
  ...
  const char* GetBackendHostname(void) { return g_strServerName.c_str(); }
